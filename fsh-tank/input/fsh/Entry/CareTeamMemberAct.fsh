...
Severity: #error
Expression: "id.nullFlavor = 'NA' or (%resource.descendants().ofType(CDA.Encounter).where(withTemplateIdOf(EncounterActivity) and id.exists($this.root = %context.id.first().root and $this.extension ~ %context.id.first().extension)))"
/*
id.nullFlavor = 'NA' or (
  %resource.descendants().ofType(CDA.Encounter).where(
    withTemplateIdOf(EncounterActivity) and
    id.exists(
      $this.root = %context.id.first().root and
      $this.extension ~ %context.id.first().extension
    )
