...
The narrative Clinical Notes required in USCDI, along with their associated LOINC codes, are outlined below. These note types are included in the [US Core Clinical Note Type value set](http://hl7.org/fhir/us/core/ValueSet/us-core-clinical-note-type), which is bound to Act.code.
