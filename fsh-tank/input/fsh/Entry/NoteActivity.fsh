...
The narrative Clinical Notes required in USCDI, along with their associated LOINC codes, are outlined below. These note types are included in the [US Core Clinical Note Type value set]({{site.data.fhir.uscore}}/ValueSet-us-core-clinical-note-type.html), which is bound to Act.code.
