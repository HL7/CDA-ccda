...
Severity: #error
Expression: "nullFlavor.exists() or entry.where(act.hasTemplateIdOf(InstructionActivity)).exists() or entry.where(observation.hasTemplateIdOf(InstructionObservation)).exists()"
